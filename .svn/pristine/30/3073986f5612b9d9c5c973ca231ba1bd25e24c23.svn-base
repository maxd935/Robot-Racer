...
class Serveur(Thread):
    """
    La classe Serveur crée un serveur TCP et permet de recevoir une
    ...
    - robot             Lien vers l'objet Robot
    - direction         sens (True = avancer, False = Reculer)
    - sock              socket du serveur recevant les commandes
    - pad               True si on est en train d'utiliser le pad, False sinon

    Les variables de classes (statiques) sont :
    - VIDEO_SOCKET      socket pour envoyer la vidéo au client connecté
    ...
    def run(self):
        ...
        while True:
            ...
            waitCreationClientServer = True
            while waitCreationClientServer:
                try:
                    Serveur.VIDEO_SOCKET = create_connection(
                        (ip, conf("network.port.stream"))
                    )
                    waitCreationClientServer = False
    ...
    def do(self, packet):
        ...
        - start     :   Démarrer le robot
                        Mettre la boucle d'analyse en position True
    ...
    def lancer_video(self, avecAnalyse):
        ...
        - avecAnalyse (bool) : Analyser l'image (et auto-controller le robot) ?
        """
        if self._robot._video_on:
            self._robot._video_on = False
            sleep(0.5)
