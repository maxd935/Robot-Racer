...
class Image:
    ...
    def fast_shift(self, where="bottom"):
        """
        Calculer rapidement le shift
        L'image seuillée doit être déjà calculée (dans self._image_thresh)

        where :
            - top : shift basé sur la partie de la ligne la plus haute visible
            - bottom : //   //   //   //   //   //   //   //     basse    //
            - center : //   //   //   //   //   //   //   //     basse    //
            - moyenne : shift moyen de toute l'image
            - mediane : shift médian sur toute l'image
        """
        height, width = self._image_thresh.shape
        img = self._image_thresh
        ligneIdx = arange(width)

        if where == "top":
            for i in range(height):
                select = img[i, :] == 255
                if select.any():
                    shift = ligneIdx[select].mean()
                    break
        elif where == "bottom":
            ...
            all = [int(ligneIdx[ligne == 255].mean())
                   for ligne in img if ligne.any()]
            if where == "moyenne":
                shift = mean(all)
            elif where == "mediane":
                shift = median(all)
            else:
                exit("image.fast_shift : where incorrect")

        # Centrage et en pourcentage
        shift = int(100 * (2 * shift / self._image_thresh.shape[1] - 1))
